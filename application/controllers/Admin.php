			...
			for ($i = 0; $i < count($result); $i++) {
				...
				if ($data['username'] == $result[$i]['username']) {
					...
					if ($data['password'] == $result[$i]['password']) {
						...
						$data['message'] = "<div class='alert bg-danger' role='alert'><svg class='glyph stroked cancel'><use xlink:href='#stroked-cancel'></use></svg>Incorrect password</div>";
					}

					break;
				}
				$data['message'] = "<div class='alert bg-danger' role='alert'><svg class='glyph stroked cancel'><use xlink:href='#stroked-cancel'></use></svg>Matching username not found</div>";
			}
